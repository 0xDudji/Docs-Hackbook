...
> Permissions are the gatekeepers of a Linux system. Misconfigured ones are a direct path to root. Learn to read them instantly and spot the misconfigs that defenders miss.

---

## The Mindset

Every file and directory on Linux has a permission set that controls who can read it, write to it, or execute it. As a pentester, you're looking for two things:
...
Permissions are the most common source of privilege escalation paths on Linux. Administrators set them once and forget them. You find them and use them.
...
| `777`  | `rwxrwxrwx` | World-writable — always suspicious |
...
Take every result and check it against **GTFOBins** (https://gtfobins.github.io/) — a curated list of Unix binaries that can be abused for privilege escalation.
...
**Pentest relevance:** `/tmp` always has the sticky bit. You can write files there but can't delete files owned by others. Use it to drop payloads and scripts.
...
for dir in $(echo $PATH | tr ':' ' '); do
  ls -ld $dir 2>/dev/null | grep -v "^d..x..x..x" | echo "$dir is writable";
done
