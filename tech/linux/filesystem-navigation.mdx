...
> On a compromised system, the files you find are often worth more than the shell itself. Config files, credentials, keys, scripts — they're all sitting there waiting.
...
Before running anything, check what's installed. Useful for planning your next move — knowing whether Python, netcat, wget, or curl are available changes your options.
...
| Command                                   | Use Case                         |
| ----------------------------------------- | -------------------------------- |
| `which <tool>`                            | Check if a binary exists on PATH |
...
| `locate id_rsa`                           | Fast search for SSH keys         |
| `find / -mmin -10 2>/dev/null`            | Files modified in last 10 min    |
| `tree -L 2 /var/www`                      | Visualize web directory layout   |
