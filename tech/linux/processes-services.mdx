...
```bash theme={"theme":{"light":"github-light","dark":"tokyo-night"}}
# 1. All running processes — who owns them?
ps aux

# 2. Any credentials in process arguments?
ps aux | grep -E "pass|secret|key|token"

# 3. All running services
systemctl list-units --type=service --state=running

# 4. What ports are listening internally?
ss -tuln | grep "127.0.0.1"

# 5. System crontab
cat /etc/crontab

# 6. All cron directories
ls -la /etc/cron.*

# 7. Current user's crontab
crontab -l

# 8. Systemd timers
systemctl list-timers --all

# 9. Writable scripts called by cron
cat /etc/crontab | awk '{print $NF}' | xargs ls -la 2>/dev/null

# 10. Real-time process monitoring (no root needed)
./pspy64
```
